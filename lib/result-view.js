...
export default class ResultView {
  ...
  constructor(marker) {
    ...
    let richCloseButton = document.createElement('div');
    ...
    openButton.onclick = () => {
      let bubbleText = this.getAllText();
      return atom.workspace.open().then(editor => editor.insertText(bubbleText));
    };
  ...
  addCopyTooltip(element) {
    return this.tooltips.add(atom.tooltips.add(element, {
      title: () => {
  ...
  addResult(result) {
    ...
    let onSuccess = ({ mimetype, el }) => {
      ...
      if (mimeType === 'text/plain') {
        ...
        let previousText = this.getAllText();
      ...
      if (this.errorContainer.getElementsByTagName('span').length === 0) {
        return this.errorContainer.classList.add('plain-error');
      } else {
        return this.errorContainer.classList.remove('plain-error');
      }
    };

    let onError = error => console.error('ResultView: Rendering error:', error);
  ...
  getAllText() {
    ...
    let resultText = this.resultContainer.innerText.trim();
    if (resultText.length > 0) {
      text += resultText;
    }

    let errorText = this.errorContainer.innerText.trim();
  ...
  buildSpinner() {
    ...
    let rect3 = document.createElement('div');
    rect3.classList.add('rect3');
    let rect4 = document.createElement('div');
    rect4.classList.add('rect4');
    let rect5 = document.createElement('div');
