...
export default class SignalListView {
  ...
  // onConfirmed: ?(KernelCommand) => void;
  panel: ?atom$Panel;
  ...
  constructor() {
    ...
    this.switchCommands = [
      { name: "Switch to", value: "switch-kernel", payload: null }
    ];
  ...
  _mapCommands(commands: Array<Object>, kernelSpec: Kernelspec) {
    return commands.map(command => ({
      ...
      payload: command.hasOwnProperty("payload") ? kernelSpec : null
    }));
  }
  ...
  async toggle() {
    ...
    if (kernel instanceof WSKernel)
      ...
      const otherKernels = await kernelManager.getAllKernelSpecsForGrammar(
        grammar,
