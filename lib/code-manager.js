...
export function getFoldRange(row) {
  ...
  if (
    range[1] < store.editor.getLastBufferRow() && getRow(range[1] + 1) === "end"
  ) {
    range[1] += 1;
  }
...
export function findPrecedingBlock(row, indentLevel) {
  ...
  while (previousRow >= 0) {
    const previousIndentLevel = store.editor.indentationForBufferRow(
      previousRow
    );
...
export function findCodeBlock() {
  ...
  const foldRange = store.editor.languageMode.rowRangeForCodeFoldAtBufferRow(
    row
  );
