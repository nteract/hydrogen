...
function getCurrentFencedCodeBlock(editor: atom$TextEditor) {
  ...
  start = new Point(start, 0);
  end = new Point(end + 1, 0);
  const cell = getCell(editor, cursor);
  return new Range(
    cell.start.isGreaterThan(start) ? cell.start : start,
    cell.end.isLessThan(end) ? cell.end : end
  );
}
