...
export function getSelectedText(editor: ?atom$TextEditor) {
  if (editor) return normalizeString(editor.getSelectedText());
}
...
export function getCommentStrings(
  ...
  // do we need this block below? No, according to package.json, if yes need types
...
export function getBreakpoints(editor: atom$TextEditor) {
  ...
  if (regexString) {
    ...
    buffer.scan(regex, ({ range }) =>
      console.log(breakpoints.push(range.start))); // Make iterator arg return void since push returns number
  }
