...
export function determineCellType(editor: atom$TextEditor, start: atom$Point) {
  const buffer = editor.getBuffer();
  const commentStartString = getCommentStartString(editor);
  if (!commentStartString) return null;
  ...
  console.log("%c looking for a match.", "color: #ff00ff;font-size: 18px;");

  buffer.scanInRange(
    regex,
    new Range(start, new Point(start.row + 1, 0)),
    ({ match, matchText }) => {
      console.log(
        "%c " + match[1] + " was selected.",
