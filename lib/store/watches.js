...
export default class WatchesStore {
  ...
  constructor(kernel: Kernel) {
    ...
    autocompleteProvider.disposables.add(this.autocompleteDisposables);
    this.addWatch();
  }
  ...
  createWatch = () => {
    ...
    if (!lastWatch || lastWatch.getCode().trim() !== "") {
      ...
      const disposable = autocompleteProvider.watchEditor(watch.editor, [
        "default",
        "workspace-center",
        "symbol-provider"
      ]);
  ...
  destroy() {
    autocompleteProvider.disposables.remove(this.autocompleteDisposables);
