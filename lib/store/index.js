...
export class Store {
   ...
   *
   * @param {atom$Point} bufferPosition - If specified, returns the embedded grammar at the buffer
   *                                      position, the one at the cursor position otherwise.
   */
  getEmbeddedGrammar(
    editor: atom$TextEditor,
    ...
    const embeddedScope = getEmbeddedScope(
      editor,
      bufferPosition ? bufferPosition : editor.getCursorBufferPosition()
    );
  ...
  /**
   * Returns a kernel attatched to an (multilanguage) ditor
   */
  getKernelForEditorBufferPosition(
    editor: atom$TextEditor,
    bufferPosition: atom$Point
  ): ?Kernel {
    const grammar = this.getEmbeddedGrammar(editor, bufferPosition);
    if (!grammar) return null;

    if (this.globalMode) {
      const currentScopeName = grammar.scopeName;
      return this.runningKernels.find(
        k => k.grammar.scopeName === currentScopeName
      );
    }
    ...
    const kernelOrMap = this.kernelMapping.get(filePath);
    if (!kernelOrMap) return null;
    if (kernelOrMap instanceof Kernel) return kernelOrMap;
    return grammar && grammar.name ? kernelOrMap.get(grammar.name) : null;
  }
}
