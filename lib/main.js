...
const Hydrogen = {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.commands.add('atom-text-editor', {
      ...
      'hydrogen:add-watch': () => {
        ...
        __guard__(this.watchSidebar, x => x.addWatchFromEditor());
      },
      'hydrogen:remove-watch': () => {
        if (!this.watchSidebarIsVisible) {
          this.toggleWatchSidebar();
        }
        __guard__(this.watchSidebar, x => x.removeWatch());
      },
  ...
  setWatchSidebar(kernel) {
    ...
    const sidebar = __guard__(kernel, x => x.watchSidebar);
    ...
    if (this.watchSidebarIsVisible) {
      __guard__(this.watchSidebar, x2 => x2.show());
    }
  },
  ...
  handleKernelCommand({ kernel, command, grammar, language, kernelSpec }) {
    ...
    if (command === 'interrupt-kernel') {
      ...
      this.kernelManager.restartRunningKernelFor(grammar, (newKernel) => {
        this.onKernelChanged(newKernel);
      });
      ...
      this.clearResultBubbles();
      this.kernelManager.destroyRunningKernelFor(grammar);
      this.kernelManager.startKernel(kernelSpec, grammar, () => {
        this.onKernelChanged(kernel);
      });
    } else if (command === 'rename-kernel') {
      __guardMethod__(kernel, 'promptRename', o => o.promptRename());
  ...
  insertResultBubble(row) {
    ...
    element.setAttribute('style',
      `margin-left: ${lineLength + 1}ch; margin-top: -${lineHeight}px`);
