...
import store, { subscriptions } from './store';
...
const Hydrogen = {
  ...
  activate() {
    ...
    subscriptions.add(atom.workspace.observeActivePaneItem((item) => {
      const editor = (item === atom.workspace.getActiveTextEditor()) ? item : null;
    ...
    subscriptions.add(atom.workspace.observeTextEditors((editor) => {
      if (isMultilanguageGrammar(editor.getGrammar())) {
        ...
        subscriptions.add(cursorSubscription);
  ...
  createResultBubble(code, row) {
    ...
    kernelManager.startKernelFor(store.grammar, (kernel) => {
