...
const Hydrogen = {
  ...
  runCell(moveDown: boolean = false) {
    ...
    console.log("%c Running cell.", "color: #ff00ff;font-size: 18px;");
    ...
    if (code) {
      ...
      if (cellType == "md" || cellType == "markdown") {
        console.log(
          "%c This cell is markdown.",
          "color: #ff00ff;font-size: 18px;"
        );
        const { markers, kernel } = store;

        new ResultView(markers, kernel, editor, endRow, true);
      } else {
        console.log("%c This cell is code.", "color: #ff00ff;font-size: 18px;");
