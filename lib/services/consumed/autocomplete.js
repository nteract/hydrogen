 ...
 *
 * @namespace _autocompleteConsumer
...
export function consumeWatchEditor(
  ...
  _autocompleteConsumer.addAutocompleteToEditor = watchEditor;
  ...
  for (let kernel of store.runningKernels) {
    ...
    _autocompleteConsumer.disposables.add(
      kernel.watchesStore.autocompleteDisposables
    );
  }

  const disposable = new Disposable(() => disableAutocomplete(store));
...
export function disableAutocomplete(store: Store) {
  ...
  _autocompleteConsumer.addAutocompleteToEditor = () => {};

  /*
   * Removes disposables from all watches (leaves references inside
   * `_autocompleteConsumer.disposables` to be disposed at the end).
   * Autocomplete is only actually disabled on dispose of `_autocompleteConsumer.disposables`
   */
  for (let kernel of store.runningKernels) {
    for (let watch of kernel.watchesStore.watches) {
  ...
  _autocompleteConsumer.disposables.dispose();
  _autocompleteConsumer.disposables = new CompositeDisposable();
}
...
export function addAutocompleteToWatch(
  ...
  const disposable = _autocompleteConsumer.addAutocompleteToEditor(
    watch.editor,
 ...
 */
export function disposeAutocompleteDisposable(disposable) {
  _autocompleteConsumer.disposables.remove(disposable);
  disposable.dispose();
}
 ...
 * @param {Disposable | CompositeDisposable} disposable
 */
export function registerAutocompleteDisposable(disposable) {
  _autocompleteConsumer.disposables.add(disposable);
}
