...
export function disableAutocomplete(store: Store) {
  ...
  // Removes autocomplete from all watches (does some shortcutting to improve performance)
  for (let kernel of store.runningKernels) {
    for (let watch of kernel.watchesStore.watches) {
 ...
 * @param {WatchStore} watch - This should always be the child `WatchStore` of `watchesStore`.
 */
...
export function removeAutocompleteFromWatch(
  ...
  if (watch.autocompleteDisposable) {
    watchesStore.autocompleteDisposables.remove(watch.autocompleteDisposable);
