...
export async function _loadNotebook(filename: string) {
  ...
  const sources = _.map(nbCells, cell => {
  ...
  let cellBreakpoints = getBreakpoints(editor);
  let cellNumber = 1;
  for (let cell of nbCells) {
    if (cell.cell_type == "code" && cell.outputs.toArray().length > 0) {
      importResult(
        { editor, markers },
        {
          ...
          row: escapeBlankRows(
            editor,
            cellBreakpoints[cellNumber - 1].row,
            cellBreakpoints[cellNumber].row - 1
          )
        }
      );
    }
...
function getCellSource(cell: Cell, commentStartString: string): string {
  ...
  return cellMarker + linesep + source;
}
