...
export async function _loadNotebook(filename: string) {
  ...
  for (let cell of nbCells) {
    hydrogenCell = toHydrogenCodeBlock(cell, commentStartString + " ");
    breakpoints.push(
      lastCellEnd + hydrogenCell.code.trim().split(linesep).length
    );
    lastCellEnd += hydrogenCell.row;
  ...
  let cellNumber = 0;
  for (let cell of nbCells) {
    const cellEnd = breakpoints[cellNumber];
...
function toHydrogenCodeBlock(cell: Cell, commentStartString: string): string {
  ...
  if (cellType === "markdown") {
    ...
    if (source.endsWith(commentStartString))
      source = source.slice(0, -commentStartString.length);
  } else {
