...
function getGrammarForNotebook(nb: Notebook) {
  const { kernelspec, kernel_info, language, language_info } = nb.metadata;
  if (!kernelspec || !kernel_info || !language || !language_info) {
    atom.notifications.addWarning(
  ...
  // This is not in nbformat, but was unofficially used before
  if (language) {
    matchedGrammar = getGrammarForLanguageName(language_info.name);
    if (matchedGrammar) return matchedGrammar;
  }

  // Officially in nbformat as of v4
  if (language_info) {
    // If language_info exists, then language_info.name is required (in v4)
    matchedGrammar = getGrammarForLanguageName(language_info.name);
    if (matchedGrammar) return matchedGrammar;

    // language_info.file_extension is not a required field, but if it exists
    // and the language name failed it is the next best option.
    if (language_info.file_extension) {
      matchedGrammar = getGrammarForFileExtension(language_info.file_extension);
    }
    if (matchedGrammar) return matchedGrammar;
  }

  // Used often in nbformat v4 however docs point to kernel_info
  if (kernelspec) {
    // Not required but better bet than name
    matchedGrammar = getGrammarForLanguageName(kernelspec.language);
    if (matchedGrammar) return matchedGrammar;

    // If kernelspec exists, then kernelspec.name is required (in v4)
    matchedGrammar = getGrammarForKernelspecName(kernelspec.name);
    if (matchedGrammar) return matchedGrammar;
  }

  // Officially in nbformat v4
  if (kernel_info) {
    // Not required but better bet than name
    matchedGrammar = getGrammarForLanguageName(kernel_info.language);
    if (matchedGrammar) return matchedGrammar;

    // If kernel_info exists, then kernel_info.name is required (in v4)
    matchedGrammar = getGrammarForKernelspecName(kernel_info.name);
    if (matchedGrammar) return matchedGrammar;
  }
...
function getGrammarForLanguageName(name: string) {
  ...
  const languageName = `language-${formattedName}`;
  const grammars = atom.grammars.getGrammars();
  const grammar = _.find(grammars, g => {
    return (
      g.name.toLowerCase() == name.toLowerCase() ||
      g.scopeName == scopeName ||
      g.packageName == languageName
    );
  });
