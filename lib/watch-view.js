...
const props = { showStatus: false, multiline: true };
...
export default class WatchView {
  constructor(kernel) {
    ...
    this.resultView = new ResultView(null, props);
  ...
  addHistorySwitch() {
    ...
    this.scrollbar.onscroll = () => {
      ...
      this.clearResults();
      this.resultView.appendOutput(
        this.currentHistory[this.currentHistory.pos]
      );
    };
  ...
  run() {
    ...
    if (code && code.length && code.length > 0) {
      this.kernel.executeWatch(code, result => {
        ...
        this.resultView.appendOutput(result);
