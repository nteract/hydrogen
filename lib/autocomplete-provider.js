...
export default function () {
  const autocompleteProvider = {
    ...
    getSuggestions({ editor, bufferPosition, prefix }) {
      const language = store.currentKernel;
      const kernel = store.currentKernel;
      const kernelClass = (kernel) ? kernel.kernelClass : null;

      if (!kernelClass || kernel.executionState !== 'idle') {
        return null;
      }
      ...
      return new Promise(resolve =>
        kernelClass.complete(prefix, ({ matches, cursor_start, cursor_end }) => {
