...
// I imported Kernel so it could be a valid parameter type
...
class KernelManager {
  // kernelspecs seems to be have this form
  /*
  kernelSpec = {
  ...
  */
  ...
  startKernelFor(grammar: atom$Grammar, onStarted: (kernel: Kernel) => void) {
    try {
      // Don't know where atom$Project is and why it doesn't have rootDirectories property
      const rootDirectory = atom.project.rootDirectories[0]
        ? atom.project.rootDirectories[0].path
        : // Should null be an option for getEditorDirectory's atom$TextEditor parameter?
  ...
  // Flow is not aware of Connection, going with Object
  startExistingKernel(
    ...
    // Expects another argument, kernelProcess: child_process$ChildProcess. Should that be nullable?
  ...
  _executeStartupCode(kernel: Kernel) {
    ...
    if (startupCode) {
      ...
      // Looks like kernel.execute hasn't been implemented. When it is, will probably want to add an onResults fn to this call
  ...
  getKernelSpecsFrom(command: string, callback: Function) {
    ...
    return exec(command, options, (err, stdout) => {
      if (!err) {
        try {
          // To avoid type error with stdout (a Buffer), threw in a quick toString
