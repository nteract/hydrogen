...
class KernelManager {
  ...
  */
  ...
  startKernelFor(grammar: atom$Grammar, onStarted: (kernel: Kernel) => void) {
    try {
      // Don't know where atom.project.rootDirectories is and why it's not found
      const rootDirectory = atom.project.rootDirectories[0]
        ? atom.project.rootDirectories[0].path
        : // Should null be an option for getEditorDirectory parameter types?
  ...
  // Flow is not aware of Connection
  startExistingKernel(
    ...
    // Expects another argument, kernelProcess: child_process$ChildProcess,. Should ZMQKernel have that be nullable?
